...
// 实际存在的死链格式
const actualDeadLinks = [
  // 语言切换链接
  '/tw/',
  '/en/',
  
  // 导航链接
  '/cn_products-services/',
  '/cn_training/',
  '/cn_university/',
  '/cn_about/',
  '/en_products-services/',
...
function removeDeadLinksInFile(filePath) {
  try {
    ...
    // 精确匹配每个死链接并移除
    actualDeadLinks.forEach(deadLink => {
      // 转义特殊字符以用于正则表达式
...
// 主函数
function main() {
  console.log('🔍 开始移除实际存在的死链接...\n');
  console.log(`📋 共有 ${actualDeadLinks.length} 个实际死链接需要移除\n`);
  ...
  console.log(`🔗 总共移除了 ${totalChanges} 个实际死链接`);
  console.log('===========================================');
